...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered)
        window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), col, g.Style.FrameRounding / 2.0f);

    float cross_extent = g.FontSize * 0.5f * 0.5071f - 1.0f;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y + 0.2), text_col, ImGuiDir_Down, 0.8f);
    }
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImDrawFlags draw_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, style.FrameRounding, draw_flags);
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, ImDrawFlags draw_flags)
{
    return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags, draw_flags);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    else
    {
        ...
        {
            const ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
            RenderFrame(frame_bb.Min, frame_bb.Max, bg_col, false, style.SelectableRounding);
        }
        RenderNavHighlight(frame_bb, id, nav_highlight_flags);
        if (flags & ImGuiTreeNodeFlags_Bullet)
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else
    {
        ...
        int num_pop_style_colors = 0;
        if (pressed || ImGui::IsItemHovered()) {
            PushStyleColor(ImGuiCol_Text, 0xFFffFFff);
            num_pop_style_colors = 1;
        }
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    {
        const float yMin = tab_bar->BarRect.Max.y;
        window->DrawList->AddRectFilled(ImVec2(tab_bar->SeparatorMinX, yMin - g.Style.TabBarBorderSize), ImVec2(tab_bar->SeparatorMaxX, yMin), col);
        const float yMax = tab_bar->BarRect.Max.y;
        window->DrawList->AddRectFilled(ImVec2(tab_bar->SeparatorMinX, yMax - g.Style.TabBarBorderSize), ImVec2(tab_bar->SeparatorMaxX, yMax), col);
    }
    return true;
}
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    const ImU32 tab_col = GetColorU32(held ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabUnfocused));
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    {
        draw_list->PathLineTo(ImVec2(bb.Min.x + 0.5f, y1));
        draw_list->PathLineTo(ImVec2(bb.Min.x + 0.5f, y2));
        draw_list->PathStroke(GetColorU32(ImGuiCol_Border), 0, g.Style.TabBorderSize);
        draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, y1));
